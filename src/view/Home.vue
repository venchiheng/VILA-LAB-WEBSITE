...
const partnerList = computed(() =>
  partnershipStore.partnerships.map(p => ({
    id: p.id,
    image: p.thumbnail ? `https://52.87.185.116:8000/storage/${p.thumbnail}` : '',
    name: p.name
  }))
)
...
const achievements = computed(() =>
  achievementStore.achievements.map(a => ({
    id: a.id,
    image: a.thumbnail ? `https://52.87.185.116:8000/storage/${a.thumbnail}` : '',
  }))
)
  ...
  try {
    ...
    projects.value = data.map(p => ({
      ...
      image: p.banner_image
        ? `https://52.87.185.116:8000/storage/${p.banner_image}`
