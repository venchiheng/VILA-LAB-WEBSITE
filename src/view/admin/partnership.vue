      ...
      <div class="dashboard-content">
        <div class="table-container">
          <div class="table-header">
            <h2 class="text-h5 font-weight-bold mb-4">Partnerships</h2>
            <div class="filters">
              <v-text-field
                v-model="searchName"
                density="compact"
...
const { partnerships, errors, loading } = storeToRefs(partnershipStore)
const formData = partnershipStore.formData
...
const handleFile = (e) => {
  ...
  partnershipStore.setThumbnail(file)
...
const submitCreate = async () => {
  try {
    await partnershipStore.createPartnership()
...
const submitUpdate = async () => {
  try {
    await partnershipStore.updatePartnership(updatingId.value, formData)
...
const deletePartnership = async (id) => {
  ...
  try {
    await partnershipStore.deletePartnership(id)
...
const getThumbnailUrl = (p) => {
  ...
  return typeof p.thumbnail === 'string'
    ? `https://52.87.185.116:8000/storage/${p.thumbnail}`
...
const filteredItems = computed(() =>
  partnerships.value.filter((p) =>
    !searchName.value || p.name.toLowerCase().includes(searchName.value.toLowerCase())
  )
)
